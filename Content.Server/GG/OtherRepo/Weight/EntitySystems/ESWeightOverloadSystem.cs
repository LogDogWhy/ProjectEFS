...
{
    [Dependency] private readonly ServerAlertsSystem _alerts = default!;
    ...
    private void OnWeightChanged(Entity<ESWeightOverloadComponent> ent, ref ESWeightChangedEvent args)
    {
        ...
        _alerts.ClearAlertCategory(ent, ent.Comp.OverloadAlertCategory);
        // ReSharper disable once SwitchStatementHandlesSomeKnownEnumValuesWithDefault
        switch (currentLevel)
        {
            case ESOverloadLevel.NoOverload:
                break;
            case ESOverloadLevel.SlightOverload:
                _alerts.ShowAlert(ent, ent.Comp.SlightOverloadAlert);
                break;
            case ESOverloadLevel.Overload:
                _alerts.ShowAlert(ent, ent.Comp.OverloadAlert);
                break;
            case ESOverloadLevel.SevereOverload:
                _alerts.ShowAlert(ent, ent.Comp.SevereOverloadAlert);
                break;
        }



        if (currentLevel == ESOverloadLevel.Overload)
        {
            ent.Comp.MovementSpeedModifier =
                Math.Clamp(1 - (args.Weight - ent.Comp.Overload) / (ent.Comp.Overload * 0.2f), 0.375f, 1.0f);
        }
        else if (currentLevel == ESOverloadLevel.SevereOverload)
        {
            ent.Comp.MovementSpeedModifier =
                Math.Clamp(1 - (args.Weight - ent.Comp.Overload) / (ent.Comp.Overload * 0.2f), 0.15f, 1.0f);
        }
        else
        {
            ent.Comp.MovementSpeedModifier =
                Math.Clamp(1 - (args.Weight - ent.Comp.Overload) / (ent.Comp.Overload * 0.2f), 0.9f, 1.0f);
        }
